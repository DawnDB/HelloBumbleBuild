...
export const metadata = { title: "HelloBumble", description: "Buzz into a better life", icons: { icon: "/favicon.ico" } };
...
<Link href="/about/hellobumble" className="absolute left-1/2 -translate-x-1/2 text-4xl font-hellobumble hover:text-palePurpleClickable">HelloBumble</Link>
...
setLoading(true);
try {
const res = await fetch("/api/newsletter", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, email }) });
if (res.ok) { setSuccess(true); setTimeout(() => { setSuccess(false); setName(""); setEmail(""); onClose(); }, 1500); }
} finally { setLoading(false); }
};
return (
<div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
<div className="bg-white/20 backdrop-blur-lg rounded-2xl p-6 w-full max-w-sm shadow-xl">
...
<>
<input className="w-full mb-3 p-3 border rounded-xl font-description bg-white/10" placeholder="Your Name" value={name} onChange={(e) => setName(e.target.value)} />
<input className="w-full mb-3 p-3 border rounded-xl font-description bg-white/10" placeholder="Your Email" value={email} onChange={(e) => setEmail(e.target.value)} />
<button onClick={submit} disabled={loading} className="w-full py-3 rounded-xl bg-pink-300/50 shadow font-description">{loading ? "Sending..." : "Subscribe"}</button>
</>
)}
<button onClick={onClose} className="mt-4 w-full text-center text-sm underline font-description">Close</button>
</div>
</div>
);
}
...
if (res.ok) { setSuccess(true); setTimeout(() => { setSuccess(false); setName(""); setEmail(""); setPhone(""); setMessage(""); onClose(); }, 1500); }
} finally { setLoading(false); }
};
return (
<div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
...
<input className="w-full mb-3 p-3 border rounded-xl font-description bg-white/10" placeholder="Your Name" value={name} onChange={(e) => setName(e.target.value)} />
<input className="w-full mb-3 p-3 border rounded-xl font-description bg-white/10" placeholder="Your Email" value={email} onChange={(e) => setEmail(e.target.value)} />
<input className="w-full mb-3 p-3 border rounded-xl font-description bg-white/10" placeholder="Your Phone" value={phone} onChange={(e) => setPhone(e.target.value)} />
<textarea className="w-full mb-3 p-3 border rounded-xl font-description bg-white/10" placeholder="Your Message" value={message} onChange={(e) => setMessage(e.target.value)} />
<button onClick={submit} disabled={loading} className="w-full py-3 rounded-xl bg-purple-300/50 shadow font-description">{loading ? "Sending..." : "Send Message"}</button>
</>
)}
<button onClick={onClose} className="mt-4 w-full text-center text-sm underline font-description">Close</button>
</div>
